...
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/jonathanp/rl_token_routing/sweep_logs/continue_v5_%j.out
#SBATCH --error=/lustre/fsw/portfolios/nvr/users/jonathanp/rl_token_routing/sweep_logs/continue_v5_%j.err
...
# MAX_CHAIN_DEPTH=4 (4h * 4 = 16h, enough to reach iter 1500).
# ============================================================
...
SCRIPT_PATH=$WORKDIR/submit_continue_v5.sh
...
NAME_SUFFIX="pretrain-mcore-qwen3-moe-megatron-A22B-lr-1e-4-minlr-1e-6-bs-1-gbs-8-seqlen-128-pr-bf16-tp-1-pp-1-cp-1-ac-sel-do-true-sp-true-ti-1500-wi-10"
...
if [ ! -d "$CELL_CKPT_ROOT" ]; then
    echo "ERROR: checkpoint root does not exist: $CELL_CKPT_ROOT" >&2
    exit 2
fi
LATEST_ITER_FILE=$CELL_CKPT_ROOT/latest_checkpointed_iteration.txt
if [ ! -f "$LATEST_ITER_FILE" ]; then
    echo "ERROR: latest_checkpointed_iteration.txt missing in $CELL_CKPT_ROOT" >&2
    exit 2
fi
LATEST_ITER=$(tr -d '[:space:]' < "$LATEST_ITER_FILE")
ITER_DIR=$(printf "%s/iter_%07d" "$CELL_CKPT_ROOT" "$LATEST_ITER")
if [ ! -d "$ITER_DIR" ]; then
...
if [ "$CURRENT_DEPTH" -lt "$MAX_CHAIN_DEPTH" ] && [ -n "${SLURM_JOB_ID:-}" ]; then
    ...
    NEXT_OUT=$LOG_DIR/continue_v5_${CELL_NAME}_d${NEXT_DEPTH}_%j.out
    NEXT_ERR=$LOG_DIR/continue_v5_${CELL_NAME}_d${NEXT_DEPTH}_%j.err
    NEXT_JOB=$(sbatch \
        --dependency=afterany:$SLURM_JOB_ID \
        --output="$NEXT_OUT" \
        --error="$NEXT_ERR" \
        --job-name="cv5_${CELL_NAME}_d${NEXT_DEPTH}" \
...
srun --container-image="$CONTAINER_IMAGE" \
     ...
     bash -c "
         ...
         cd $WORKDIR
         sh run_mcore_qwen3.sh dlc A22B 1 8 1e-4 1e-6 128 128 bf16 1 1 1 1 8 true true true false sel false 1500 \\

#!/bin/bash
#SBATCH --job-name=highbs_decode_v2
...
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/jonathanp/highbs_decode_v2_%j.out
set -uo pipefail
...
echo "=== EP=$TP HIGH-BS DECODE v2 (env var pattern for engine args) ==="
...
srun --nodes=1 --ntasks=1 -w "$HEAD" --overlap --mem=0 \
  ...
  bash -c "
   $PIP
   export TMPDIR=/tmp/hbd2_\$SLURM_JOB_ID VLLM_NO_USAGE_STATS=1 DO_NOT_TRACK=1
   ...
   for CFG in pretrained_235b=$PRE r15_cp4682=$R15; do
     ...
     for BS in 2048 8192 16384; do
       echo === \$CNAME bs=\$BS ===
       # Use the patched env vars instead of CLI flags
       ...
       python3 $SCRIPT --models \${CNAME}_ep${TP}=\$CPATH --tp-size $TP \
         ...
         --output $RES/ep${TP}_highbs_v2_\${CNAME}_bs\${BS}_\$SLURM_JOB_ID.json || echo \"RC=\$?\"
     done
   done

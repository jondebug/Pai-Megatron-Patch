...
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/jonathanp/climb_M2_%j.out
set -uo pipefail
...
# Climb M curve with reduced KV pre-allocation to free room for activations.
srun --nodes=1 --ntasks=1 --gpus-per-node=8 --mem=0 \
     --container-image=$CONTAINER --container-mounts=$BASE:$BASE --container-workdir=$DIR \
  bash -c '
   export TMPDIR=/tmp/cm2_$SLURM_JOB_ID VLLM_NO_USAGE_STATS=1 DO_NOT_TRACK=1 VLLM_WORKER_MULTIPROC_METHOD=spawn
   mkdir -p $TMPDIR
   ...
   python3 '$SCRIPT' --models pre_ep8='$PRE',r15_ep8='$R15' --tp-size 8 \
     ...
     --output '$RES'/climb2_M1024_${SLURM_JOB_ID}.json || echo "M=1024 RC=$?"
   ...
   python3 '$SCRIPT' --models pre_ep8='$PRE',r15_ep8='$R15' --tp-size 8 \
     ...
     --num-warmup 3 --num-trials 30 --cuda-graphs \
     --output '$RES'/climb2_M2048_${SLURM_JOB_ID}.json || echo "M=2048 RC=$?"

   # Run 3: M=4096 (knee). bs=64 plen=1024 = 65536 tokens/forward.
   export VLLM_MAX_NUM_BATCHED_TOKENS=65536
   python3 '$SCRIPT' --models pre_ep8='$PRE',r15_ep8='$R15' --tp-size 8 \
     --prompt-lengths 1024 --batch-sizes 64 --max-tokens 4 \
     --num-warmup 3 --num-trials 30 --cuda-graphs \
     --output '$RES'/climb2_M4096_${SLURM_JOB_ID}.json || echo "M=4096 RC=$?"
  '
echo "done rc=$?"

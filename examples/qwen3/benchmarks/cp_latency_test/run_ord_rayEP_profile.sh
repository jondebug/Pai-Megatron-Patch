#!/bin/bash
#SBATCH --job-name=ord_rayEP_prof
...
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/jonathanp/ord_rayEP_prof_%x_%j.out
#SBATCH --error=/lustre/fsw/portfolios/nvr/users/jonathanp/ord_rayEP_prof_%x_%j.err

# Phase C: multi-node Ray vLLM (EP=NODES*8, IB) with per-rank kernel profiling +
# a batch sweep. Mirrors run_ord_walltime_rayEP.sh but adds VLLM_TORCH_PROFILER_DIR
# + a profiled burst, and an inter-node all-to-all microbench BEFORE booting vLLM.
# Env: MODEL, MODELNAME, PROFBS(default 64), SWEEP_BS(default 8,64), set --nodes=N.
set -uo pipefail
...
echo "=== Ray vLLM PROFILE nodes=$NN TP/EP=$TP model=$MODELNAME head=$HEAD $(date) ==="
...
srun --nodes=1 --ntasks=1 -w "$HEAD" --overlap --mem=0 \
  ...
  bash -c "
   $PIP
   export TMPDIR=/tmp/rayEPp_\$SLURM_JOB_ID VLLM_NO_USAGE_STATS=1 DO_NOT_TRACK=1
   ...
   PDIR=$RES/trace_${MODELNAME}_ep${TP}_\${SLURM_JOB_ID}; mkdir -p \$PDIR
   # batch sweep (unprofiled)
   timeout 1500 python3 $SCRIPT --models ${MODELNAME}_ep${TP}=$MODEL --tp-size $TP --distributed-executor-backend ray \
     --prompt-lengths 256 --batch-sizes $SWEEP_BS --max-tokens 128 --num-warmup 3 --num-trials 8 \
     --output $RES/vllm_ep${TP}_sweep_${MODELNAME}_\${SLURM_JOB_ID}.json
   RC1=\$?; echo SWEEP_RC=\$RC1
   # profiled burst
   export VLLM_TORCH_PROFILER_DIR=\$PDIR
   timeout 1500 python3 $SCRIPT --models ${MODELNAME}_ep${TP}=$MODEL --tp-size $TP --distributed-executor-backend ray \
     --prompt-lengths 256 --batch-sizes $PROFBS --max-tokens 32 --num-warmup 3 --num-trials 3 \
     --profile --profile-steps 32 \
     --output $RES/vllm_ep${TP}_profcfg_${MODELNAME}_\${SLURM_JOB_ID}.json

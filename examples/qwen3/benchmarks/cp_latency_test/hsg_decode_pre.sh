...
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/jonathanp/hsg_dec_pre_%j.out
set -uo pipefail
...
srun --nodes=1 --ntasks=1 -w "$HEAD" --overlap --mem=0 \
  ...
  bash -c "
   $PIP
   export TMPDIR=/tmp/hsgD_\$SLURM_JOB_ID VLLM_NO_USAGE_STATS=1 DO_NOT_TRACK=1
   ...
   python3 $SCRIPT --models pre_ep${TP}=$PRE --tp-size $TP \
     ...
     --num-warmup 2 --num-trials 20 --cuda-graphs \
     --output $RES/hsg_dec_pre_\$SLURM_JOB_ID.json

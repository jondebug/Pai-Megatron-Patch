...
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/jonathanp/no_autotune2_%j.out
set -uo pipefail
...
srun --nodes=1 --ntasks=1 --gpus-per-node=8 --mem=0 --container-image=$CONTAINER --container-mounts=$BASE:$BASE --container-workdir=$DIR bash -c 'export TMPDIR=/tmp/na2_$SLURM_JOB_ID VLLM_NO_USAGE_STATS=1 DO_NOT_TRACK=1 VLLM_WORKER_MULTIPROC_METHOD=spawn TORCHINDUCTOR_MAX_AUTOTUNE=0 TORCHINDUCTOR_COORDINATE_DESCENT_TUNING=0 TORCHINDUCTOR_BENCHMARK_FUSION=0; mkdir -p $TMPDIR; python3 '$SCRIPT' --models pre_ep8='$PRE',r15_ep8='$R15' --tp-size 8 --prompt-lengths 8192 --batch-sizes 1 --max-tokens 8 --num-warmup 3 --num-trials 30 --cuda-graphs --output '$RES'/no_autotune2_${SLURM_JOB_ID}.json'
echo done rc=$?

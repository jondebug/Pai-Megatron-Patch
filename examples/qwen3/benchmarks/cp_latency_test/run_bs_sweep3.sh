...
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/jonathanp/bs_sweep3_%j.out
set -uo pipefail
...
# Re-run of bs_sweep2 minus r05 (HF tokenizer error on 3rd model).
# Two cells × bs={2,4,8,16,32} × 30 trials at plen=8192 prefill.
srun --nodes=1 --ntasks=1 --gpus-per-node=8 --mem=0 \
     --container-image=$CONTAINER --container-mounts=$BASE:$BASE --container-workdir=$DIR \
  bash -c '
   export TMPDIR=/tmp/bs3_$SLURM_JOB_ID VLLM_NO_USAGE_STATS=1 DO_NOT_TRACK=1 VLLM_WORKER_MULTIPROC_METHOD=spawn
   mkdir -p $TMPDIR
   python3 '$SCRIPT' --models pre_ep8='$PRE',r15_ep8='$R15' --tp-size 8 \
     ...
     --output '$RES'/bs_sweep3_${SLURM_JOB_ID}.json
  '
echo "done rc=$?"

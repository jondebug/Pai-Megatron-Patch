#!/bin/bash
# HSG decode sweep: EP=8 (2 nodes × 4 GPU) — decode-regime bs sweep at plen=256
# Reproduces our critical ORD findings (bs=512 EP=64 +15%, bs=8192 -12%) on GB200/NVLink
#SBATCH --job-name=hsg_decode_ep8
...
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/jonathanp/hsg_decode_ep8_%j.out
set -uo pipefail
...
srun --nodes=1 --ntasks=1 -w "$HEAD" --overlap --mem=0 \
  ...
  bash -c "
   $PIP
   export TMPDIR=/tmp/hsg_dec_\$SLURM_JOB_ID VLLM_NO_USAGE_STATS=1 DO_NOT_TRACK=1
   ...
   python3 $SCRIPT --models pre_ep${TP}=$PRE,r15_ep${TP}=$R15 --tp-size $TP \
     ...
     --prompt-lengths 256 --batch-sizes 1,8,64,256,512,1024 --max-tokens 32 \
     --num-warmup 3 --num-trials 20 --cuda-graphs \
     --output $RES/hsg_decode_ep${TP}_\$SLURM_JOB_ID.json

...
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/jonathanp/rl_token_routing/benchmark_logs/eval235b_%x_%j.out
#SBATCH --error=/lustre/fsw/portfolios/nvr/users/jonathanp/rl_token_routing/benchmark_logs/eval235b_%x_%j.err

# Required env:
#   HF_OUTPUT_DIR  - path to the already-converted HF dir (must have 118 safetensors)
#   ITER_NUM       - iteration number (for benchmark dir naming)
#   LIMIT          - lm-eval limit (1000 or empty for full)
#   RUN_NAME       - cell name for logging
#   TRAINED_MEGATRON_CKPT - checkpoint root (for benchmark dir path)

set -euo pipefail

CONTAINER_IMAGE=/lustre/fsw/portfolios/nvr/users/jonathanp/containers/pai-megatron-patch_25.04.sqsh

HF_OUTPUT_DIR="${HF_OUTPUT_DIR:?Set HF_OUTPUT_DIR}"
...
echo "============================================================"
echo "EVAL-ONLY: ${RUN_NAME} iter=${ITER_NUM} limit=${LIMIT_TAG}"
echo "Job: ${SLURM_JOB_ID:-manual}  Node: ${SLURM_JOB_NODELIST:-unknown}"
echo "HF: ${HF_OUTPUT_DIR}"
echo "============================================================"

# Verify HF has safetensors
NSHARDS=$(ls "${HF_OUTPUT_DIR}"/*.safetensors 2>/dev/null | wc -l)
if [ "${NSHARDS}" -lt 100 ]; then
    echo "ERROR: only ${NSHARDS} safetensors in ${HF_OUTPUT_DIR} (expected 118)"; exit 1
fi
echo "HF OK: ${NSHARDS} shards"

# Skip if accuracy already exists
if [ -f "${BENCHMARK_DIR}/accuracy_summary.json" ]; then
    echo "accuracy_summary.json already exists, skipping."
    exit 0
fi
...
srun --ntasks=1 --nodes=1 \
     ...
     bash -c "
         ...
         python3 -m lm_eval --model hf \
             --model_args pretrained='${HF_OUTPUT_DIR}',parallelize=True,dtype=bfloat16,trust_remote_code=True \

...
rm -f $SENT
OUT_JSON=$RES/hsg_${TAG}_nsys_agrs_dp${DP_TOTAL}_$SLURM_JOB_ID.json
...
for N in "${NODES[@]}"; do
  ...
  srun --nodes=1 --ntasks=1 -w $N --overlap --mem=0 --gpu-bind=none \
    ...
    bash -c "$COMMON_ENV; export CUDA_VISIBLE_DEVICES=0,1,2,3; \
      nsys profile -t cuda,nvtx -s none --cpuctxsw=none --cuda-graph-trace=node --force-overwrite=true \
        -o $NSYS_DIR/${TAG}_dp${DP_TOTAL}_${SLURM_JOB_ID}_\$(hostname) \
        $SERVE_CMD & NP=\$!; \
      while [ ! -f $SENT ]; do sleep 5; kill -0 \$NP 2>/dev/null || exit 1; done; \
      kill -INT \$NP; for i in \$(seq 1 72); do kill -0 \$NP 2>/dev/null || break; sleep 10; done; pkill -9 -f \"python.*vllm\" 2>/dev/null; wait \$NP; echo \"nsys finalized on \$(hostname)\"" &
  PIDS+=($!)
  RANK=$((RANK + 4))
  sleep 5
done

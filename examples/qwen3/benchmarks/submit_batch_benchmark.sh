...
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/jonathanp/rl_token_routing/benchmark_logs/bench_batch_%j.out
#SBATCH --error=/lustre/fsw/portfolios/nvr/users/jonathanp/rl_token_routing/benchmark_logs/bench_batch_%j.err

# Batch benchmark: runs multiple checkpoints sequentially in one SLURM job.
# Usage: sbatch submit_batch_benchmark.sh --manifest /path/to/manifest.json [--limit 1000]
#
# manifest.json format:
# [
#   {"checkpoint_dir": "/path/to/ckpt", "run_name": "name", "wandb_run_id": "id"},
#   ...
# ]
...
echo "============================================================"
echo "BATCH BENCHMARK: ${TOTAL} checkpoints"
...
for idx, entry in enumerate(manifest):
    ...
    if not safetensors:
        print(f'  Converting Megatron -> HF...')
        os.chdir('${CONVERTOR_DIR}')
        rc = os.system(
            f'bash scripts/qwen3/run_8xH20.sh A3B '
    ...
    cmd = [
        ...
        '--model', 'hf',
        '--model_args', f'pretrained={hf_output},trust_remote_code=True,dtype=bfloat16',
        '--device', 'cuda:0',
        '--tasks', tasks,

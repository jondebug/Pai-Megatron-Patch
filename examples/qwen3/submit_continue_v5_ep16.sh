#!/bin/bash
#SBATCH --job-name=continue_v5_ep16
...
#SBATCH --output=/lustre/fsw/portfolios/nvr/users/jonathanp/rl_token_routing/sweep_logs/continue_v5_ep16_%j.out
#SBATCH --error=/lustre/fsw/portfolios/nvr/users/jonathanp/rl_token_routing/sweep_logs/continue_v5_ep16_%j.err
...
SCRIPT_PATH=$WORKDIR/submit_continue_v5_ep16.sh
...
if [ "$CURRENT_DEPTH" -lt "$MAX_CHAIN_DEPTH" ] && [ -n "${SLURM_JOB_ID:-}" ]; then
    ...
    NEXT_OUT=$LOG_DIR/continue_v5_ep16_${CELL_NAME}_d${NEXT_DEPTH}_%j.out
    NEXT_ERR=$LOG_DIR/continue_v5_ep16_${CELL_NAME}_d${NEXT_DEPTH}_%j.err
    NEXT_JOB=$(sbatch \
        --dependency=afterany:$SLURM_JOB_ID \
        --output="$NEXT_OUT" \
        --error="$NEXT_ERR" \
        --job-name="cv5e16_${CELL_NAME}_d${NEXT_DEPTH}" \

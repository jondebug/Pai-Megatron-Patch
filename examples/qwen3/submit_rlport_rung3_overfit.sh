#!/bin/bash
#SBATCH --job-name=cp_rlport_rung3
...
# =============================================================================
# RL-PORT RUNG-3 OVERFIT PROBE ("does the objective descend")
# =============================================================================
# Copy of submit_rlport_capture.sh. Trains 8 iters with LOO_BETA=0.008 (denser
# credit; 0.03 saturates) so the graduation gate can read whether policy_loss /
# cov_ptlp_adv move in the improving direction and cov_ptlp_adv > 0.
#
# FIXED-BATCH NOTE: a truly fixed/repeated batch would require a data-loader hack
# (Megatron draws a fresh shuffled batch each --train-iters step). Per the task's
# accepted fallback, this runs ON-POLICY 8 iters (each step sees a new batch), so
# monotonic descent is NOT guaranteed; read direction via cov_ptlp_adv>0 and the
# policy_loss early-vs-late trend from graduation_gate.py.
#
...
#   * SEED=1234
#   * RL_TRAIN_ITERS=8      -> rung-3 overfit probe
...
#
# Launch:
#   RUN_NAME=235bv21math_rlport_rung3 sbatch submit_rlport_rung3_overfit.sh
# =============================================================================
set -euo pipefail
...
RL_TRAIN_ITERS="${RL_TRAIN_ITERS:-8}"    # RUNG-3 overfit probe (NOTE: on-policy, see header)
SEED="${SEED:-1234}"

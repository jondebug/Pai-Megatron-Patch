#!/bin/bash
#SBATCH --job-name=cp_rlport_capture
...
# =============================================================================
# RL-PORT ON-TOPOLOGY CAPTURE (G7 sum-gate + Rung-1/Rung-2 causal capture)
# =============================================================================
# Derived verbatim from submit_fresh_corner_ep16_math.sh. This is a SHORT
# forward+few-step CAPTURE (RL_TRAIN_ITERS=15), NOT a training run. It exercises
# the committed H1 (Gumbel-top-k + Plackett-Luce) + H2 (global loo_smoothmax)
# estimator on the real EP=16 topology so the reviewer can read the G7 sum-gate
# and the RL causal-capture telemetry from the .out.
#
...
#   * SEED=1234
#   * RL_TRAIN_ITERS=15     -> capture only
...
#   * --rl-loo-beta 0.03    -> at 235B counts beta*load must be O(1) (NOT 0.3)
#
# Launch:
#   RUN_NAME=235bv21math_rlport_capture sbatch submit_rlport_capture.sh
# =============================================================================
set -euo pipefail
...
RL_TRAIN_ITERS="${RL_TRAIN_ITERS:-15}"   # SHORT capture, NOT training
SEED="${SEED:-1234}"
...
LOO_BETA="${LOO_BETA:-0.03}"              # O(1) beta*load at 235B (NOT 0.3)
